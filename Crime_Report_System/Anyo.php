<?php
require "Config.php";
$topic = $description = $name = $gender = $dob = $doi = $city = $state = $suspect = $place = $present=  "";
$topic_err = $description_err = $name_err = $gender_err = $dob_err = $doi_err = $city_err = $state_err = $suspect_err = $place_err = $present_err = "";
...
//if ther are no errors, go ahead and insert into the database
  if(empty($topic_err) && empty($description_err) && empty($dob_err) && empty($doi_err) && empty($place_err)  && empty($present_err) && empty($city_err) && empty($state_err) && empty($gender_err)){
    $sql = "INSERT INTO anonymous_details ( name, dob, gender, doi, suspect, place, city, state, present, topic, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_prepare($conn, $sql);
    if($stmt){
        mysqli_stmt_bind_param($stmt, "sssssssssss",  $param_name,  $param_dob, $param_gender, $param_doi, $param_suspect, $param_place, $param_city, $param_state, $param_present, $param_topic, $param_description );
...
<body>
  ...
  <div class="popup">
    ...
    <hr class="ruler">
    <p>
        Reporting Anonymously keeps your id confidential. To track your Report an unique id will be provided. It is not  mandatory to mention your name while Reporting Anonymously.
    </p>
