...
//if ther are no errors, go ahead and insert into the database
  if(empty($topic_err) && empty($description_err) && empty($name_err) && empty($email_err) && empty($mobile_err) && empty($dob_err) && empty($doi_err) && empty($place_err) && empty($present_err) && empty($city_err) && empty($state_err) && empty($gender_err))
  
    $sql = "INSERT INTO report_details (Id, name, email, mobileno, dob, gender, doi, suspect, place, city, state, present, topic,proof, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, $pname, ?)";
